...
// Helper function to check for dangerous property names that could lead to prototype pollution
function isUnsafePropertyName(key) {
...
module.exports = function onValue(model, node, seed, depth, outerResults,
    ...
    if (isJSONG) {
        ...
        for (i = 0, len = optimizedLength - 1; i < len; i++) {
            ...
            // Skip unsafe property names to prevent prototype pollution
    ...
    // reporting the value.
        ...
        if (valueNode !== undefined) {
          if (k != null) {
              hasValues = true;
              // Skip unsafe property names to prevent prototype pollution
              if (!isUnsafePropertyName(k) && !curr[k]) {
                curr[k] = valueNode;
              }
          } else {
